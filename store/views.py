...
from .utils import test_login_required
...
def product_detail_view(requests, pk):
    ...
    if user.is_authenticated:
        if requests.method == 'POST':
            ...
            return redirect(f'/{requests.path[1:3]}/store/product_detail/{pk}')
    else:
    ...
    if cur_product_rated is not None:
        ...
        cur_product_rated_full = 0
        cur_product_rated_part = 0
...
def cart_view(requests):
    ...
    if referer == 'http://127.0.0.1:8000/store/men_products/':
        return redirect(f'/{requests.path[1:3]}/store/men_products/#{product_id}')
    elif referer == 'http://127.0.0.1:8000/store/women_products/':
        return redirect(f'/{requests.path[1:3]}/store/women_products/#{product_id}')
    elif referer == 'http://127.0.0.1:8000/store/baby_products/':
        return redirect(f'/{requests.path[1:3]}/store/baby_products/#{product_id}')
    elif referer == f'http://127.0.0.1:8000/store/product_detail/{product_id}':
        return redirect(f'/{requests.path[1:3]}/store/product_detail/{product_id}')
    else:
        return redirect(f'/{requests.path[1:3]}/store/#{product_id}')
...
def checkout_view(requests):
    if requests.method == 'POST':
        ...
        if data.get('promo_code') != '' and promo is None:
            ...
            if card_objs:
                total_quantity = CardObj.objects.filter(user=user, ordered=False).aggregate(Sum('quantity'))[
                                     'quantity__sum'] or 0
        ...
        if data.get('payment') == 'online':
            ...
            product_quantity = CardObj.objects.filter(user=requests.user, ordered=False).aggregate(Sum('quantity'))[
                'quantity__sum']
            ...
            return redirect(f'/{requests.path[1:3]}/checkout')
...
def like_obj_view(requests):
    ...
    if 'men_products' in referer:
        ...
        return redirect(f'/{requests.path[1:3]}/store/baby_products/#{product_id}')
    elif f'/{requests.path[1:3]}/store/product_detail/{product_id}' in referer:
        return redirect(f'/{requests.path[1:3]}/store/product_detail/{product_id}')
    elif 'wishlist' in referer:
        return redirect(f'/{requests.path[1:3]}/store/wishlist')
    else:
        return redirect(f'/{requests.path[1:3]}/#{product_id}')
...
def update_cart_view(request):
    ...
    total_quantity = CardObj.objects.filter(user=request.user, ordered=False).aggregate(Sum('quantity'))[
                         'quantity__sum'] or 0
...
def pay_online_view(request, pk):
    if request.method == 'POST':
        ...
        if int(data.get('expire_month')) > 12:
            expired_month_error = 'it must be exist month: 1 to 12'
            check += 1
        if int(data.get('expire_month')) != pan.expired_month:
            expired_month_not_match_error = 'Month is not match'
            check += 1

        if int(data.get('expire_year')) > pan.expired_year or int(
                data.get('expire_year')) < pan.expired_year:
...
def delete_order_view(request, pk):
    ...
    for i in order.product.all():
        ...
        if card is None:
            ...
            new_cart = CardObj.objects.create(user=request.user, product=i, name=i.name, price=i.price,
                                              discount=i.discount,
...
def support_view(request):
    if request.user.is_authenticated:
        ...
        cart_counter = 0
    return render(request, 'support.html', context={'cart_num': cart_counter})
...
def faq_view(request):
    if request.user.is_authenticated:
        ...
        cart_counter = 0
    return render(request, 'faq.html', context={'cart_num': cart_counter})
...
def privacy_view(request):
    if request.user.is_authenticated:
        ...
        cart_counter = 0
    return render(request, 'privacy.html', context={'cart_num': cart_counter})
...
def about_view(request):
    if request.user.is_authenticated:
        ...
        cart_counter = 0
    return render(request, 'about.html', context={'cart_num': cart_counter})
...
def search_view(request):
    ...
    if not products:
        nothing_found = 'product not found'
...
def suggestions_view(request):
    ...
    if query:
        ...
        suggestions_list = [product.name for product in products]
